...
function obtenerUltimaSemanaConDatos(PDO $pdo): string
{
    ...
    $result = ejecutarConsulta($pdo, "SELECT MAX(production_date) as max_fecha FROM milk_production");
    if (!empty($result) && $result[0]['max_fecha']) $fechas[] = $result[0]['max_fecha'];

    $result = ejecutarConsulta($pdo, "SELECT MAX(slaughter_date) as max_fecha FROM slaughter_records");
    if (!empty($result) && $result[0]['max_fecha']) $fechas[] = $result[0]['max_fecha'];

    $result = ejecutarConsulta($pdo, "SELECT MAX(production_date) as max_fecha FROM egg_production");
    if (!empty($result) && $result[0]['max_fecha']) $fechas[] = $result[0]['max_fecha'];
...
function obtenerGallinasActivas(PDO $pdo, string $fechaLimite): int
{
    $result = ejecutarConsulta($pdo,
        "SELECT quantity FROM chicken_inventory
