...
<div class="max-w-7xl mx-auto">
    ...
    <!-- 1. Género -->
    <select name="gender" id="genderSelect" required class="border rounded-lg p-2">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Etiquetas existentes pasadas desde PHP
    const existingTags = <?= json_encode($existingTags ?? []) ?>;

    function generarCodigo(prefix) {
        // Filtrar tags que empiezan con el prefijo
        ...
        // Siguiente número con 3 dígitos (ej: 001, 002, ...)
        const nextNum = String(maxNum + 1).padStart(3, '0');
        return prefix + nextNum;
    }

    genderSelect.addEventListener('change', function() {
        const genero = this.value; // 'H' o 'M'
        if (genero === 'H') {
            tagInput.value = generarCodigo('H');
        } else if (genero === 'M') {
            tagInput.value = generarCodigo('M');
        } else {
            tagInput.value = '';
        }
    });
