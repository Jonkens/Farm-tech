...
// Mapeo de pestañas a nombres de variable de página
$pages = [];
...
$generos = ['H'=>'Hembra','M'=>'Macho'];
$data['razas'] = obtenerRazas($pdo);
/////////////////////
$tiposAnimales = obtenerTiposAnimales($pdo);
/////////////////////
$data['tiposAnimal'] = ejecutarConsulta($pdo, "SELECT id, name FROM animal_types ORDER BY name");
$data['proveedores'] = obtenerProveedores($pdo);
//$data['animalesSelect'] = obtenerAnimalesPaginados($pdo, 50, 0); // para selects
$data['animalesSelect'] = ejecutarConsulta($pdo, 
    "SELECT id, name, animal_type_id FROM animals WHERE status != 'sacrificado' ORDER BY name"
);
$data['alimentosSelect'] = obtenerAlimentos($pdo);

// Datos paginados según pestaña
switch ($tab) {
    case 'vacas':
    ...
    // NUEVO: pasar todas las etiquetas existentes a JavaScript
    ...
    case 'sacrificios':
        ...
        // Línea ~185 en index.php (dentro del case 'sacrificios')
        $data['disponibles'] = ejecutarConsulta($pdo,
        "SELECT a.id, a.tag, a.name, b.name AS breed, a.weight_kg 
        FROM animals a 
