...
<style>
  ...
  .inp {
    width:100%; padding:9px 13px; border:1.5px solid #ecdbaa; border-radius:9px; font-size:14px;
    background: #fffef7; color:#2d3748; transition:border .15s,box-shadow .15s; font-family:inherit;
  }
  .inp:focus {
    outline:none; border-color:#b87c4f; background:#fff; box-shadow:0 0 0 3px rgba(184,124,79,.12);
  }
  .lbl {
    display:block; font-size:11px; font-weight:700; color:#5a3e1b;
    text-transform:uppercase; letter-spacing:.06em; margin-bottom:5px;
  }
  .sec {
    font-size:11px; font-weight:700; color:#2d6a4f; text-transform:uppercase;
    letter-spacing:.1em; padding-bottom:9px; border-bottom:2px solid #e6f4ec; margin-bottom:18px;
  }
  .btn {
    background: linear-gradient(135deg,#2d6a4f,#1f4d38); color:#fff; padding:12px 28px;
  ...
  .estado-r:checked + .estado-l {
    opacity:1; transform:scale(1.06); box-shadow:0 0 0 2px #2d6a4f;
  }
  .estado-l {
    opacity:.55; transition:all .2s; cursor:pointer; padding:5px 14px;
    border-radius:100px; font-size:12px; font-weight:700; border:1.5px solid; display:inline-block;
  }

  /* Para impresión (solo por si alguien imprime el formulario) */
  @media print {
    body {
...
<div class="max-w-4xl mx-auto">
  ...
  <div>
    <h2 class="text-2xl font-bold text-white">Nuevo Informe Médico</h2>
    <p class="text-gray-200 text-sm mt-0.5">Complete los datos para generar el reporte veterinario en PDF</p>
  </div>
  <div class="text-right">
    <div class="text-xs text-gray-200 uppercase tracking-wider font-semibold">Fecha</div>
    <div class="text-white font-semibold text-sm"><?= date('d/m/Y') ?></div>
  </div>
</div>
  ...
  <form method="POST" id="reporteForm">
    ...
    <div class="card p-6 mb-5">
      ...
      <div id="meds">
        <div class="med-grid mb-2" id="med-0">
          ...
          <button type="button" onclick="rmMed('med-0')"
    ...
    <div class="flex justify-end gap-3">
      <button type="button" onclick="limpiarFormulario()" class="btn-limpiar px-5 py-3 font-semibold rounded-xl transition-colors text-sm">
        Limpiar
      </button>
...
function limpiarFormulario() {
  // Limpiar todos los campos del formulario (excepto el número de informe readonly)
  const form = document.getElementById('reporteForm');
  const elements = form.elements;
  for (let i = 0; i < elements.length; i++) {
    const el = elements[i];
    if (el.readOnly) continue;
    if (el.type === 'text' || el.type === 'textarea' || el.type === 'number' || el.type === 'date') {
      el.value = '';
    } else if (el.type === 'select-one') {
      el.selectedIndex = 0;
    } else if (el.type === 'radio') {
      // Desmarcar todos los radio, luego marcar el 'estable' por defecto
      if (el.name === 'estado') {
        el.checked = false;
      }
    }
  }
  // Establecer el radio 'estable' como marcado por defecto
  const estableRadio = document.querySelector('input[name="estado"][value="estable"]');
  if (estableRadio) estableRadio.checked = true;

  // Fecha actual en el campo fecha de consulta
  ...
  // Limpiar medicamentos: mantener solo la primera fila vacía
  const contenedorMeds = document.getElementById('meds');
  while (contenedorMeds.children.length > 1) {
    contenedorMeds.removeChild(contenedorMeds.lastChild);
  }
  // Vaciar los valores de la primera fila
  const primeraFila = document.getElementById('med-0');
  if (primeraFila) {
    const inputs = primeraFila.querySelectorAll('input');
    inputs.forEach(inp => inp.value = '');
  }
  mc = 1; // Reiniciar contador de medicamentos
}
</script>
</body>
</html>